...
Oledssd1306Display::Oledssd1306Display(GeneralSettings &pSettings,
                                       WifiTask        &pWifi,
                                       I2C             &pI2C) :
  CO2MonitorDisplay(pSettings),
  _wifi(pWifi), _i2c(pI2C)
{
...
void
Oledssd1306Display::RenderReadings()
{
    // if(!_ssd) return;
    // char ppm[10];
    // if(_values.CO2)
    ...
    //     }
    //      _ssd->DrawText(FontSize::Medium,128-((strlen(temp)+1)*10),0,
    //      "%sC",temp);
    //      _ssd->DrawText(FontSize::Medium,128-((strlen(humidity)+1)*10),17,
    //      "%s%c",humidity,'%');
    //      _ssd->DrawText(FontSize::Medium,128-((strlen(pressure)+3)*10),34,
    //      "%shPa",pressure); _ssd->DrawText(FontSize::Small,2,57, "IP
    //      %s",_wifi.GetManager().GetIPAddress().c_str());
...
void
Oledssd1306Display::RenderAccessPointInfo(const std::string &pSSID,
...
bool
Oledssd1306Display::IsPresent()
{
    return false; //_ssd!=nullptr;
}

bool
Oledssd1306Display::SupportsHalfOn()
{
    return false;
}

void
Oledssd1306Display::On(bool pHalf)
{
    // _ssd->On();
}

void
Oledssd1306Display::Off()
{
    //  _ssd->Off();
}