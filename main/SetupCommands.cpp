...
SaveSettingsCommand::SaveSettingsCommand(GeneralSettings &pSettings) :
  _settings(pSettings)
{
}
void
SaveSettingsCommand::Process(Json &pJson, Json &pResult)
{
    ...
    if (pJson.HasProperty("PrimaryNtpServer"))
        _settings.SetPrimaryNtpServer(
            pJson.GetStringProperty("PrimaryNtpServer"));
...
LoadSettingsCommand::Process(Json &pJson, Json &pResult)
{
    ...
    pResult.CreateBoolProperty("EnablePowerSave",
                               _settings.GetEnablePowerSave());
    pResult.CreateStringProperty("PrimaryNtpServer",
                                 _settings.GetPrimaryNtpServer());
    pResult.CreateStringProperty("SecondaryNtpServer",
                                 _settings.GetSecondaryNtpServer());
...
GetSystemInfoCommand::GetSystemInfoCommand(GeneralSettings &pSettings) :
  _settings(pSettings)
{
}
...
GetSystemInfoCommand::Process(Json &pJson, Json &pResult)
{
    ...
    pResult.CreateNumberProperty("LeastFreeHeap",
                                 (int)System::GetLeastHeapFreeSinceBoot());
}

std::string
GetSystemInfoCommand::GetName()
{
    return "SYSTEMINFO";
}
...
GetAvailableWifiNetworksCommand::Process(Json &pJson, Json &pResult)
{
    ...
    for (auto availableNetwork : availableNetworks)
    {
        ...
        snprintf(apMacAddr, sizeof(apMacAddr), "%.2X:%.2X:%.2X:%.2X:%.2X:%.2X",
                 ...
                 availableNetwork->bssid[4], availableNetwork->bssid[5]);
...
std::string
GetAvailableWifiNetworksCommand::GetName()
{
    return "GETNETWORKS";
}

GetCurrentWifiNetworkCommand::GetCurrentWifiNetworkCommand(
    GeneralSettings &pSettings) :
  _settings(pSettings)
{
}
...
std::string
GetCurrentWifiNetworkCommand::GetName()
{
...
void
SelectWifiNetworkCommand::TestConnectToNetwork()
{
...
void
SelectWifiNetworkCommand::SetConnectToNetwork()
{
...
void
SelectWifiNetworkCommand::Process(Json &pJson, Json &pResult)
{
    if (pJson.HasProperty("Mode"))
    {
            ...
            for (auto configuredNetwork : _wifi.GetNetworks())
            {
                ...
                if (configuredNetwork.second->ssid == connectionInfo->ssid)
                {
                    ...
                    connectionJson->CreateNumberProperty(
                        "channel", (int)connectionInfo->channel);
                    connectionJson->CreateStringProperty(
                        "ipv4Address", connectionInfo->ipv4Address);
                    connectionJson->CreateStringProperty(
                    ...
                    connectionJson->CreateStringProperty("dnsServers",
                                                         dnsServers);
                    connectionJson->CreateStringProperty("ntpServers",
                                                         ntpServers);
        ...
        else if (mode == "Remove")
        {
            ...
            if (ssid == connectionInfo->ssid ||
                !_wifi.RemoveConfiguration(ssid))
            {
                delete connectionInfo;
                pResult.CreateBoolProperty("Status", false);
                return;
...
std::string
SelectWifiNetworkCommand::GetName()
{
...
void
DataManagementCommand::Process(Json &pJson, Json &pResult)
{
    if (pJson.HasProperty("Mode"))
    {
        auto mode = pJson.GetStringProperty("Mode");
        if (mode == "GetValues")
        {
            ...
            for (auto groupPair : ValueController::GetCurrent().GetGroups())
            {
                for (auto keyPair : groupPair.second->SourcesByName)
                {
                    ...
                    if (source->GetFlags() != VALUESOURCE_NOFLAGS &&
                        source->GetDataType() != ValueDataType::String &&
                        source->GetDataType() != ValueDataType::Bool)
                    {
                        Json *availJson = new Json();
                        availJson->CreateStringProperty("Group",
                                                        groupPair.first);
            ...
            pResult.CreateNumberProperty(
                "SensorUpdateInterval",
                (int)_settings.GetSensorUpdateInterval());
        ...
        else if (mode == "SetValues")
        {
            if (pJson.HasArrayProperty("Values"))
            {
                for (const auto &group :
                     ValueController::GetCurrent().GetGroups())
                {
                ...
                {
                    if (setElement->HasProperty("Group") &&
                        setElement->HasProperty("Name"))
                    {
                        auto group = setElement->GetStringProperty("Group");
                        auto name = setElement->GetStringProperty("Name");

                        if (!ValueController::GetCurrent().GetGroups().contains(
                                group))
                        {
                            continue;
                        }
                        auto valueGroup =
                            ValueController::GetCurrent().GetGroups()[group];
                        if (!valueGroup->SourcesByName.contains(name))
                            continue;
                        auto valueContainer = valueGroup->SourcesByName[name];
                        valueContainer->DefaultSource->SetIsIncludedInDatalog(
                            true);
                    }
                }
        ...
        else if (mode == "GetFlashInfo")
        {
            ...
            for (auto i = 0; i < _manager.GetNumBuckets(); i++)
            {
                ...
                {
                    bucketJson->CreateNumberProperty(
                        "Offset", (int)currentBucketInfo.info.Offset);
                    bucketJson->CreateNumberProperty(
                        "Index", (int)currentBucketInfo.info.Index);
                    if (currentBucketInfo.info.BlockStartTime !=
                        0xFFFFFFFFFFFFFFFF)
                        bucketJson->CreateNumberProperty(
                            "BlockStartTime",
                            (double)currentBucketInfo.info.BlockStartTime);
                    if (currentBucketInfo.info.BlockEndTime !=
                        0xFFFFFFFFFFFFFFFF)
                        bucketJson->CreateNumberProperty(
                            "BlockEndTime",
                            (double)currentBucketInfo.info.BlockEndTime);
                    bucketJson->CreateNumberProperty(
                        "DataLength",
                        (double)currentBucketInfo.info.DataLength);
                    bucketJson->CreateNumberProperty(
                        "PayloadOffset", (int)currentBucketInfo.offset);
                    bucketJson->CreateNumberProperty(
                        "NumReadings", (int)currentBucketInfo.numReadings);
                    bucketJson->CreateNumberProperty(
                        "CurrentTime", (double)currentBucketInfo.currentTime);
                ...
                else
                {
                    ...
                    printf(
                        "Offset: %x, Index: %x, Start: %x, End: %x, DL: %X\n",
                        (int)bucket.Offset, (int)bucket.Index,
                        (int)bucket.BlockStartTime, (int)bucket.BlockEndTime,
                        (int)bucket.DataLength);
                    bucketJson->CreateNumberProperty("Offset",
                                                     (int)bucket.Offset);
                    bucketJson->CreateNumberProperty("Index",
                                                     (int)bucket.Index);
                    if (bucket.BlockStartTime != 0xFFFFFFFFFFFFFFFF)
                        bucketJson->CreateNumberProperty(
                            "BlockStartTime", (double)bucket.BlockStartTime);
                    if (bucket.BlockEndTime != 0xFFFFFFFFFFFFFFFF)
                        bucketJson->CreateNumberProperty(
                            "BlockEndTime", (double)bucket.BlockEndTime);
                    bucketJson->CreateNumberProperty("NumReadings",
                                                     (int)bucket.NumReadings);
                    bucketJson->CreateNumberProperty("DataLength",
                                                     (double)bucket.DataLength);
                    buckets.push_back(bucketJson);
                }
            }
...
void
MqttManagementCommand::Process(Json &pJson, Json &pResult)
{
    if (pJson.HasProperty("Mode"))
    {
        auto mode = pJson.GetStringProperty("Mode");
        if (mode == "GetValues")
        {
            ...
            pResult.CreateStringProperty("ServerAddress",
                                         _mqttManager.GetServerAddress());
            pResult.CreateStringProperty("Username",
                                         _mqttManager.GetUsername());
            pResult.CreateStringProperty("Password",
                                         _mqttManager.GetPassword());
            pResult.CreateStringProperty("ReadingsTopicPath",
                                         _mqttManager.GetReadingsTopic());
            pResult.CreateStringProperty("InfoTopicPath",
                                         _mqttManager.GetInfoTopic());
            pResult.CreateNumberProperty("PublishDelay",
                                         _mqttManager.GetPublishDelay());
            pResult.CreateArrayProperty("Available", availableValues);
        ...
        {
            if (pJson.HasArrayProperty("ReadingsTopic") &&
                pJson.HasArrayProperty("InfoTopic"))
            {
                for (const auto &group :
                     ValueController::GetCurrent().GetGroups())
                {
                ...
                for (auto setElement : readingsElements)
                {
                        ...
                        setElement->HasProperty("Name"))
                    {
                        ...
                        if (!valueGroup->SourcesByName.contains(name))
                            continue;
                        auto valueContainer = valueGroup->SourcesByName[name];
                        valueContainer->DefaultSource
                            ->SetIsIncludedInMQTTReadings(true);
                    }
                }
                ...
                for (auto setElement : infoElements)
                {
                        ...
                        setElement->HasProperty("Name"))
                    {
                        ...
                        if (!valueGroup->SourcesByName.contains(name))
                            continue;
                        auto valueContainer = valueGroup->SourcesByName[name];
                        valueContainer->DefaultSource->SetIsIncludedInMQTTInfo(
                            true);
                    }
                }
                ValueController::GetCurrent().Save();
            ...
            if (pJson.HasProperty("ServerAddress"))
                _mqttManager.SetServerAddress(
                    pJson.GetStringProperty("ServerAddress"));
            ...
            if (pJson.HasProperty("ReadingsTopicPath"))
                _mqttManager.SetReadingsTopic(
                    pJson.GetStringProperty("ReadingsTopicPath"));

            if (pJson.HasProperty("InfoTopicPath"))
                _mqttManager.SetInfoTopic(
                    pJson.GetStringProperty("InfoTopicPath"));

            if (pJson.HasProperty("PublishDelay"))
                _mqttManager.SetPublishDelay(
                    pJson.GetIntProperty("PublishDelay"));
