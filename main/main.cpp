...
MainLogicLoop::MainLogicLoop()
{
    ...
    _sensorManager = new SensorManager(*_devicePersonality, *_generalSettings,
                                       *_i2c, *_dataManager);
    ...
    if (displayConfig.Enabled)
    {
        ...
        display = _screenDrivers->LoadDriver(displayConfig.Driver,
    ...
    AddValueSource(new ValueSource(*this, SYSTEM_TIMESTRING, String,
                                   Dimensionless, _valTimeString,
                                   VALUESOURCE_NOFLAGS));
    AddValueSource(new ValueSource(*this, SYSTEM_VERSION, String, Dimensionless,
                                   _valVersion,
                                   VALUESOURCE_NOFLAGS | DEFAULT_MQTTINFO));
    AddValueSource(new ValueSource(*this, SYSTEM_BUILD, String, Dimensionless,
                                   _valBuild,
                                   VALUESOURCE_NOFLAGS | DEFAULT_MQTTINFO));
    AddValueSource(new ValueSource(*this, SYSTEM_BATTERYVOLTS, Fixed,
                                   Dimensionless, _valBatteryVolts,
                                   GET_LATEST_DATA | DEFAULT_MQTT));
...
MainLogicLoop::ProcessOnUIThread()
{
    ...
    if (_sensorManager->GetLastSensorRead() > _lastRead &&
...
MainLogicLoop::Run()
{
    ...
    {
        sntp = new SntpManager(_generalSettings->GetNtpServers(),
                               _generalSettings->GetEnableDhcpNtp());
        sntp->Init();
    }
...
std::vector<int>
MainLogicLoop::ResolveTimeSeries(std::string pName,
                                 uint32_t    pSecondsInPast,
                                 uint32_t    pSteps)
{
    ...
    if (curTime % (pSecondsInPast / pSteps))
        curTime +=
            (pSecondsInPast / pSteps) - (curTime % (pSecondsInPast / pSteps));
    ...
    while (read > 0)
    {
        ...
        {
            auto step =
                ((resultData.GetTimeStamps()[i] - startTime) * multiplier) /
                perStep;
...
const std::string &
MainLogicLoop::GetValuesSourceName() const
{
