...
CalibrationCommand::CalibrationCommand(GeneralSettings &pSettings) :
  _settings(pSettings)
{
}

void
CalibrationCommand::Process(Json &pJson, Json &pResult)
{
    auto success = false;
    if (pJson.HasProperty("Mode"))
    {
        ...
        if (mode == "Info")
        {
                ...
                for (auto keyPair : groupPair.second->SourcesByName)
                {
                    ...
                    if (source->GetFlags() & CALIBRATION_INFO)
                    {
                        if (groupJson == nullptr)
                            groupJson =
                                pResult.CreateObjectProperty(groupPair.first);
            ...
            pResult.CreateNumberProperty("BackgroundCO2",
                                         _settings.GetBackgroundCO2());
            success = true;
        }
        else if (mode == "Perform")
        {
            auto method = ValueController::GetCurrent().GetMethod(
                GROUP_CO2, CO2METHOD_CALIBRATE);
            if (method)
            {
        ...
        {
            if (pJson.HasProperty("EnableABC") &&
                pJson.HasProperty("ABCHoursPerCycle") &&
                pJson.HasProperty("Baseline"))
            {
                ...
                {
                    auto method = ValueController::GetCurrent().GetMethod(
                        GROUP_CO2, CO2METHOD_DISABLEABC);
                    method->Invoke({});
                    success = true;
                }
                if (success)
